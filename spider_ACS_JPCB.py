...
def spider_ACS_JPCB(ISS):
    url = ("https://pubs.acs.org/loi/jpcbfk")
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://pubs.acs.org/toc/jpcbfk/" + str(int(iss[0])) + "/" +str(int(iss[1])))
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\ACS_JPCB.csv", "data\\Temp\\ACS_JPCB.csv")
    print(len(paper_url_list))
