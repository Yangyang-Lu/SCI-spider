...
def get_page(url):
    ...
    pattern2 = r'.*/journal/nano-today/vol/(.*)/suppl/C.*'
    vol1 = int(str(re.findall(pattern2, str(each_paper_url[0]))).replace("['", '').replace("']", ""))
    vol = ('Vol. ' + str(vol1) + "-" + str(vol1 - 1) + "-" + str(vol1 - 2))
    url = []
    url.append("https://www.sciencedirect.com/journal/nano-today/vol/" + str(vol1) + "/suppl/C")
    url.append("https://www.sciencedirect.com/journal/nano-today/vol/" + str(vol1 - 1) + "/suppl/C")
    url.append("https://www.sciencedirect.com/journal/nano-today/vol/" + str(vol1 - 2) + "/suppl/C")
    #print(url)
    return str(vol), url
...
def spider_ELSEVIER_Nanotoday(ISS):
    url = ("https://www.sciencedirect.com/journal/nano-today/issues")
    ...
    if latest_issue == ISS:
        ...
        url_list.append(str(latest_issue_url[2]))
        print(url_list)
    else:
        url_list = []
        url_list.append(str(latest_issue_url[0]))
        ...
        url_list.append("https://www.sciencedirect.com/journal/nano-today/vol/" + str(int(iss[0])) + "/suppl/C")
        url_list.append("https://www.sciencedirect.com/journal/nano-today/vol/" + str(int(iss[1])) + "/suppl/C")
        url_list.append("https://www.sciencedirect.com/journal/nano-today/vol/" + str(int(iss[2])) + "/suppl/C")
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\ELSEVIER_Nanotoday.csv", "data\\Temp\\ELSEVIER_Nanotoday.csv")
    print(len(paper_url_list))
