...
def get_page(url):
    ...
    #print(k)
    latest_issue_inf = ("Vol. "+str(k[1])+", Iss. "+str(k[2]))
...
def spider_ACS_NANO(ISS):
    url = ("https://pubs.acs.org/loi/ancac3")
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://pubs.acs.org/toc/ancac3/" + str(int(iss[0])) + "/" +str(int(iss[1])))
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\ACS_NANO.csv", "data\\Temp\\ACS_NANO.csv")
    print(len(paper_url_list))
