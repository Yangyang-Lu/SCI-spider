...
def spider_ACS_JPCL(ISS):
    url = ("https://pubs.acs.org/loi/jpclcd")
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://pubs.acs.org/toc/jpclcd/" + str(int(iss[0])) + "/" +str(int(iss[1])))
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\ACS_JPCL.csv", "data\\Temp\\ACS_JPCL.csv")
    print(len(paper_url_list))
