...
def get_page(url):
    ...
    each_paper_url = soup.findAll("span", {'class': "accordion-title js-accordion-title"})
    #print(each_paper_url[0])
    #print(each_paper_url[1])
    pattern2 = r'.*Volumes (.*)</span>.*'
    pattern1 = r'\d+'
    vol11 = []
    for url in each_paper_url:
        vvv = str(re.findall(pattern2, str(url))).replace("['", '').replace("']", "")
        if vvv != []:
            vol11.append(vvv)

    vol = ('Vol. ' + str(int((re.findall(pattern1, str(vol11[1])))[1])) + "-" + str(int((re.findall(pattern1, str(vol11[1])))[1])-1))
    url = []
    url.append("https://www.sciencedirect.com/journal/carbon/vol/" + str(int((re.findall(pattern1, str(vol11[1])))[1])) + "/suppl/C")
    url.append("https://www.sciencedirect.com/journal/carbon/vol/" + str(int((re.findall(pattern1, str(vol11[1])))[1])-1) + "/suppl/C")
    #print(url)
    return str(vol), url
...
def spider_ELSEVIER_Carbon(ISS):
    url = ("https://www.sciencedirect.com/journal/carbon/issues")
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://www.sciencedirect.com/journal/carbon/vol/" + str(int(iss[0])) + "/suppl/C")
        url_list.append("https://www.sciencedirect.com/journal/carbon/vol/" + str(int(iss[1])) + "/suppl/C")
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\ELSEVIER_Carbon.csv", "data\\Temp\\ELSEVIER_Carbon.csv")
    print(len(paper_url_list))
