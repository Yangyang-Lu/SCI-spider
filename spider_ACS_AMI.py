...
def spider_ACS_AMI(ISS):
    url = ("https://pubs.acs.org/loi/aamick")
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://pubs.acs.org/toc/aamick/" + str(int(iss[0])) + "/" +str(int(iss[1])))
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\ACS_AMI.csv", "data\\Temp\\ACS_AMI.csv")
    print(len(paper_url_list))
