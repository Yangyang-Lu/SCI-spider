...
def get_page(url):
    ...
    latest_issue_url = ("https://advances.sciencemag.org"+str(url))
...
def get_latest_issue_inf(url):
    ...
    for paper in each_paper_url:
        ...
        paper_url_list.append("https://advances.sciencemag.org" + str(str(re.findall(pattern1, str(paper))).replace("[", "").replace("]", "").replace("'", "")))
...
def spider_AAAS_ScienceAdvances(ISS):
    url = ("https://advances.sciencemag.org/content/by/year")
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://advances.sciencemag.org/content/" + str(int(iss[0])) + "/" + str(int(iss[1])))
        print(url_list)
    paper_url_list = []
    paper_title_list = []
    for url in url_list:
    ...
    print(len(paper_url_list))
    [paper_url_list, paper_title_list] = compare_url_title(paper_url_list, paper_title_list, "data\\AAAS_ScienceAdvances.csv", "data\\Temp\\AAAS_ScienceAdvances.csv")
    print(len(paper_url_list))
