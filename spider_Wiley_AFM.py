...
def get_abstract(url):
    content = get_content(url)
    soup = BeautifulSoup(content, 'html.parser')
    #print(len(soup.findAll("meta", {'property': "og:title"})))
...
def spider_Wiley_AFM(ISS):
    url = ("https://www.onlinelibrary.wiley.com/loi/16163028")
    [latest_issue, latest_issue_url] = get_page(url,ISS)
    ...
    if latest_issue == ISS:
        ...
        url_list.append("https://www.onlinelibrary.wiley.com/toc/16163028/" + str(1990+int(iss[0])) + "/" + str(int(iss[0])) + "/" + str(int(iss[1])))
        print(url_list)
    paper_url_list = []
    for url in url_list:
    ...
    paper_url_list = compare_url(paper_url_list, "data\\Wiley_AFM.csv", "data\\Temp\\Wiley_AFM.csv")
    print(len(paper_url_list))
